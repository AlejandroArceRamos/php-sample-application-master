...
BEGIN
    DECLARE uuid BINARY(36);
    DECLARE ordered BINARY(16);
    SET uuid = UUID();
    SET ordered = UNHEX(CONCAT(SUBSTR(uuid, 15, 4), SUBSTR(uuid, 10, 4), SUBSTR(uuid, 1, 8), SUBSTR(uuid, 20, 4), SUBSTR(uuid, 25)));
    RETURN ordered;
END;
//
DELIMITER ;
